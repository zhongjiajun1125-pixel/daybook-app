...
export type ScenarioTemplate = WorkspaceTemplate & {
  id: string
  chip: string
  prompt: string
  keywords: string[]
...
export const STORAGE_KEY = "trace-workspace-v2"
...
export function createEmptyExecutionProgress(): ExecutionProgress {
  return {
    todayChecked: [],
    weekChecked: [],
    updatedAt: null
...
export function getNextRunNumber(records: WorkspaceRecord[]) {
...
export function toggleEntry(list: string[], item: string) {
  return list.includes(item) ? list.filter((entry) => entry !== item) : [...list, item]
}

export function pickScenario(text: string) {
...
export function buildGenericTemplate(text: string): WorkspaceTemplate {
...
export function buildBlueprintFromTemplate(
  ...
  const selectedPathId =
    preferredPathId && template.paths.some((path) => path.id === preferredPathId)
      ? preferredPathId
  ...
  return {
    ...
    thoughtLabel: template.thoughtLabel,
    analysis: {
...
export function buildBlueprintFromText(text: string, mode: Mode, preferredPathId?: string | null) {
  const matchedScenario = pickScenario(text)
  const template = matchedScenario ?? buildGenericTemplate(text)

  return buildBlueprintFromTemplate(template, mode, preferredPathId)
}

export function buildRecordFromBlueprint(
  thought: string,
  mode: Mode,
  ...
  return {
    ...
    analysisProvider: options.analysisProvider ?? "heuristic",
    thought: {
      label: blueprint.thoughtLabel,
      body: thought
    },
    analysis: blueprint.analysis,
    assumptions: blueprint.assumptions,
    paths: blueprint.paths,
    selectedPathId: blueprint.selectedPathId,
    planBase: blueprint.planBase,
    resources: blueprint.resources,
    execution: buildExecutionMap(blueprint.paths, options.execution),
    review: {
...
export function buildRecordFromText(
  text: string,
  mode: Mode,
  options: {
    id: string
  ...
  const blueprint = buildBlueprintFromText(text, mode, options.preferredPathId)

  return buildRecordFromBlueprint(text, mode, blueprint, options)
}
...
export function getSelectedPath(record: WorkspaceRecord) {
  return record.paths.find((path) => path.id === record.selectedPathId) ?? record.paths[0]
}

export function getExecutionProgress(record: WorkspaceRecord, pathId = record.selectedPathId) {
  return record.execution[pathId] ?? createEmptyExecutionProgress()
}

export function deriveActionPlan(record: WorkspaceRecord, pathId = record.selectedPathId) {
...
export function getCompletionSummary(record: WorkspaceRecord, pathId = record.selectedPathId) {
...
export function hydrateStoredRecord(record: WorkspaceRecord) {
  return {
    ...record,
    analysisProvider: record.analysisProvider ?? "heuristic",
